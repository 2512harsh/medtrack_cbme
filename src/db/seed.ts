...
async function seed() {
  ...
  if (existing.length > 0) {
    console.log("Database already has subjects — skipping seed. Nothing was changed.");
    return;
  }
  ...
  const departmentRows = await db
    ...
    .returning();
  const departmentIdByName = new Map(departmentRows.map((d) => [d.name, d.id]));
  ...
  console.log(
    `Seeded: 1 institution, ${departmentRows.length} departments, 1 stream, ${yearRows.length} professional years, ${subjectRows.length} subjects, ${topicRows.length} topics, ${subtopicRows.length} subtopics, ${COMPETENCY_DEFS.length} competencies.`
  );
}
