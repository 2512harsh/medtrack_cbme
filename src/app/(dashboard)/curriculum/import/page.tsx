...
type ImportStage = "idle" | "uploading" | "parsing" | "validating" | "ready" | "importing" | "success" | "partial" | "failed" | "cancelled";

const recentImports = [
  { id: "1", date: "Aug 5, 2026", user: "Dr. Anand Sharma", file: "competencies_q2_2026.xlsx", records: 248, status: "Success", duration: "3m 12s", result: "248 created · 0 updated" },
  { id: "2", date: "Jul 28, 2026", user: "Prof. Meera Reddy", file: "obgyn_update.csv", records: 56, status: "Partial", duration: "1m 48s", result: "54 created · 2 errors" },
  { id: "3", date: "Jul 15, 2026", user: "Dr. Anand Sharma", file: "mbbs_prof1_competencies.xlsx", records: 412, status: "Success", duration: "5m 04s", result: "412 created" },
  { id: "4", date: "Jun 30, 2026", user: "Prof. Meera Reddy", file: "biochemistry_revision.xlsx", records: 0, status: "Failed", duration: "32s", result: "Validation failed" },
];
...
export default function ExcelImportPage() {
  ...
  const [stream, setStream] = useState("mbbs");
  const [professionalYear, setProfessionalYear] = useState("py-1");
    ...
    switch (stage) {
      ...
      case "importing":
        return (
          <div className="flex items-center gap-2 rounded-lg border bg-card px-4 py-3 text-sm">
            <Loader2 className="h-4 w-4 animate-spin text-primary" />
            Importing records into the curriculum...
          </div>
        );
      case "success":
        return (
          <div className="flex items-center gap-2 rounded-lg border bg-card px-4 py-3 text-sm text-emerald-700">
            <CheckCircle2 className="h-4 w-4" />
            Import completed successfully. All {validationSummary.rows} records were processed.
          </div>
        );
      case "partial":
        return (
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Excel Import"
        description="Import or update official competency data from Excel files"
        ...
        actions={
          <>
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              onClick={() => downloadCsv("competency_import_sample.csv", SAMPLE_CSV)}
            >
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Drag & drop a spreadsheet containing competency data
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1.5">
                  ...
                  <Select value={stream} onValueChange={(v) => setStream(v ?? "mbbs")} disabled={isBusy}>
                    <SelectTrigger className="w-full" id="stream">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={professionalYear} onValueChange={(v) => setProfessionalYear(v ?? "py-1")} disabled={isBusy}>
                    <SelectTrigger className="w-full" id="professionalYear">
                      <SelectValue placeholder="Select year" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="py-1">First Professional MBBS</SelectItem>
                      <SelectItem value="py-2">Second Professional MBBS</SelectItem>
                      <SelectItem value="py-3">Third Professional MBBS</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
          ...
          <Card ref={historyRef}>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                A history of recent import operations
              </p>
            </CardHeader>
